...
if [ -n "${DBC_INSTALLER_BASE_URL:-}" ]; then
    ...
    INSTALLER_BASE_URL="${DBC_INSTALLER_BASE_URL:-https://dbc.columnar.tech}"
fi

if [ -n "${DBC_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$DBC_DOWNLOAD_URL"
...
if [ -n "${DBC_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$DBC_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi

if [ -n "${DBC_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$DBC_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi

if [ -n "${DBC_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$DBC_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi

UNMANAGED_INSTALL="${DBC_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
fi
AUTH_TOKEN="${DBC_GITHUB_TOKE:-}"
...
RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/dbc"
...
download_binary_and_run_installer() {
    ...
    say "Downloading $APP_NAME ($APP_VERSION) for ${_arch}" 1>&2
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in
        "aarch64-apple-darwin")
            ...
            _archive="dbc-darwin-amd64-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            ...
            # if [ -n "$_archive" ]; then
                ...
                # return 0
            # fi
            _archive="dbc-windows-amd64-${APP_VERSION}.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="dbc-windows-x86-${APP_VERSION}.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-unknown-linux-gnu")
            _archive="dbc-linux-arm64-${APP_VERSION}.tar.gz"
            ...
            _archive="dbc-linux-arm64-musl-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "arm-unknown-linux-gnueabihf")
            _archive="dbc-linux-arm-gnueabihf-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "arm-unknown-linux-musl-dynamiceabihf")
            _archive="dbc-linux-arm-musleabihf-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "armv7-unknown-linux-gnueabihf")
            _archive="dbc-linux-armv7-gnueabihf-${APP_VERSION}.tar.gz"
            ...
            _archive="dbc-linux-armv7-musleabihf-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "armv7-unknown-linux-musl-dynamic")
            _archive="dbc-linux-armv7-musleabihf-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            ...
            ;;
        "armv7-unknown-linux-musl-staticabihf")
            _archive="dbc-linux-armv7-musleabihf-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
        ...
        "i686-unknown-linux-gnu")
            _archive="dbc-linux-x86-${APP_VERSION}.tar.gz"
            if ! check_glibc "2" "17"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="dbc-linux-x86-musl-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "powerpc64-unknown-linux-gnu")
            _archive="dbc-linux-powerpc64-${APP_VERSION}.tar.gz"
            if ! check_glibc "2" "17"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
        ...
        "powerpc64le-unknown-linux-gnu")
            _archive="dbc-linux-powerpc64le-${APP_VERSION}.tar.gz"
            if ! check_glibc "2" "17"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
        ...
        "riscv64gc-unknown-linux-gnu")
            _archive="dbc-linux-riscv64-${APP_VERSION}.tar.gz"
        ...
        "s390x-unknown-linux-gnu")
            _archive="dbc-linux-s390x-${APP_VERSION}.tar.gz"
            if ! check_glibc "2" "17"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
        ...
        "x86_64-unknown-linux-gnu")
            _archive="dbc-linux-amd64-${APP_VERSION}.tar.gz"
            if ! check_glibc "2" "17"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="dbc-linux-amd64-musl-${APP_VERSION}.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
downloader() {
    ...
    if [ "$SUCCESS" -ne 0 ] || [ "$HTTP_STATUS" -eq 403 ]; then
        say "Error: dbc is not available for your platform. Please contact support@columnar.tech for assistance."
        exit 1
    fi
}
