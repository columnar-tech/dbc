...
async function main() {
  ...
  out.search = JSON.parse(await globalThis.dbcSearch(handle, ""));

  // resolve versions + latest package URL for a driver/platform
  out.resolve = JSON.parse(await globalThis.dbcResolve(handle, "test-driver-1", "linux_amd64"));

  // install to disk (download -> temp -> extract -> manifest + symlink)
  const installDir = fs.mkdtempSync(path.join(os.tmpdir(), "dbc-install-"));
  out.install = JSON.parse(await globalThis.dbcInstall(handle, "test-driver-1", installDir));
  ...
  const concurrent = await Promise.all(
    Array.from({ length: 5 }, () => globalThis.dbcSearch(handle, "test-driver-1").then((s) => JSON.parse(s).drivers.length))
  );
